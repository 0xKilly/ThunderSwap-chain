...
import {IThunderPair} from "./interfaces/IThunderPair.sol";
import {IThunderRouter02} from "./interfaces/IThunderRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by ThunderSwap teams.
 */
contract ThunderZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped ETH (WETH)
    IWETH public WETH;

    // ThunderRouter interface
    IThunderRouter02 public thunderRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from ThunderSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address ThunderRouter
    address private thunderRouterAddress;
     ...
     * @param _thunderRouter: address of the ThunderRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WETHAddress,
        address _thunderRouter,
        uint256 _maxZapReverseRatio
    ) {
        WETHAddress = _WETHAddress;
        WETH = IWETH(_WETHAddress);
        thunderRouterAddress = _thunderRouter;
        thunderRouter = IThunderRouter02(_thunderRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap ETH in a WETH pool (e.g. WETH/token)
     * @param _lpToken: LP token address (e.g. THUNDER/ETH)
     * @param _tokenAmountOutMin: minimum token amount (e.g. THUNDER) to receive in the intermediary swap (e.g. ETH --> THUNDER)
     */
     ...
     * @param _lpToken: LP token address (e.g. THUNDER/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. THUNDER) in the intermediary swap (e.g. BUSD --> THUNDER)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. THUNDER/WETH)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in ETH/WETH) in the intermediary swap (e.g. THUNDER --> ETH)
     */
    function zapOutETH(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. THUNDER/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. THUNDER or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. THUNDER) in the intermediary swap (e.g. BUSD --> THUNDER)
     */
    ...
    {
        address token0 = IThunderPair(_lpToken).token0();
        address token1 = IThunderPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IThunderPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = thunderRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = thunderRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IThunderPair(_lpToken).token0() || _token0ToZap == IThunderPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IThunderPair(_lpToken).token0() || _token1ToZap == IThunderPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IThunderPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IThunderPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IThunderPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = thunderRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = thunderRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IThunderPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "ThunderRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "ThunderRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        thunderRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), thunderRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(thunderRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = thunderRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = thunderRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = thunderRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = thunderRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = thunderRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
